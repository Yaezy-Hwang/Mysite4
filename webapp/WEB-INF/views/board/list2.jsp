			...
			<div id="board">
				<div id="list">
					...
					<table>
						...
						<tbody>
							<c:forEach items="${pMap.boardList}" var="post">
								<tr>
									<td>${post.ro}</td>
					...
					<div id="paging">
						<ul>
							<c:if test="${pMap.prev eq true}">
								<li><a href="${pageContext.request.contextPath}/board/list2?crtPage=${pMap.endPageBtnNo-5}">◀</a></li>
							</c:if>
							
							<c:forEach var="page" begin="${pMap.startPageBtnNo}" end="${pMap.endPageBtnNo}" step="1">
								<li <c:if test="${param.crtPage eq page}"> class="active" </c:if>>
									<a href="${pageContext.request.contextPath}/board/list2?crtPage=${page}">${page}</a>
								</li>
							</c:forEach>
							
							<c:if test="${pMap.next eq true}">
								<li><a href="${pageContext.request.contextPath}/board/list2?crtPage=${pMap.startPageBtnNo+5}">▶</a></li>
							</c:if>
						</ul>
